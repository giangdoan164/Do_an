...
class ZREPORT extends TCPDF
{
    ...
    function convert_entity($string)
    {
        $arr_values = array(
            '&#185;'  => '<sup>1</sup>',
            '&#x00B9' => '<sup>1</sup>',
        ...
        foreach ($arr_values as $k => $v)
        {
            $arr_search[$k] = $k;
            $arr_replace[$k] = $v;
            $decoded_k = html_entity_decode($k);
            $arr_search[$decoded_k] = $k;
            $arr_replace[$decoded_k] = $v;
        }
        return str_replace(array_values($arr_search), array_values($arr_replace), $string, $count);
    }
